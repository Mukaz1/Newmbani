...
import { CreateLandlordDto, PostLandlordDto, UpdateLandlordDto } from '../dtos/lanlords.dto';
import { CustomHttpResponse } from '../../common';
import { ExpressQuery, HttpResponseInterface, HttpStatusCodeEnum, Landlord, PaginatedData, SystemEventsEnum, User } from '@newmbani/types';
...
export class LandlordsService {
    ...
    async getAllLandlords(query?: ExpressQuery): Promise<HttpResponseInterface> {
       ...
       */
      async getLandlordById(landlordId: string): Promise<HttpResponseInterface<Landlord>> {
        try {
          ...
          const landlords = await LandlordModel.aggregate<Landlord>(aggregation).exec();
          if (!landlords.length) {
            return new CustomHttpResponse({
              statusCode: HttpStatusCodeEnum.NOT_FOUND,
              message: 'There is no landlord with the id you have provided. Try again!',
              data: null,
            });
          }
      ...
      async updateLandlord(
        ...
        try {
          ...
          const { _id, createdBy, createdAt, __v, ...update } =
            payload;
