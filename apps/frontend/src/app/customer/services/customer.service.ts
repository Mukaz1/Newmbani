...
export class CustomersService extends SocketService {
  ...
  /**
   * Emits a customer search event over a socket connection.
   */
  ...
  createCustomer(
    ...
    return this.httpClient.post<HttpResponseInterface<Customer>>(endpoint, payload);
  }
    ...
    const options = data
      ? {
          params: new HttpParams()
            .set('limit', String(data.limit))
            .set('page', String(data.page))
            .set('slim', String(data.slim ?? false))
            .set('keyword', data.keyword ?? ''),
        }
      : {};
    return this.httpClient.get<HttpResponseInterface<PaginatedData<Customer[]>>>(endpoint, options);
  }
   ...
   * Note: Adjust endpoint or add utility to API_ENDPOINTS if needed.
   */
  getCustomerByPhone(
    phone: string
  ): Observable<HttpResponseInterface<Customer>> {
    const endpoint = `${API_ENDPOINTS.GET_CUSTOMERS}/phone/${phone}`;
    return this.httpClient.get<HttpResponseInterface<Customer>>(endpoint);
  }
  ...
  updateCustomer(
    ...
    return this.httpClient.patch<HttpResponseInterface<Customer>>(endpoint, payload);
  }
}
