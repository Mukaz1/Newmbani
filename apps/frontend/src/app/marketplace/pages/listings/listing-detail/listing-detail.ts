...
export class ListingDetail implements OnInit {
  ...
  similarProperties = signal<PropertyListing[]>([]);
  ...
  propertyImages = computed(() => this.listing()?.property?.images);
  ...
  constructor() {
    this.metaService.setMeta({
      ...
      title: 'View Property',
      description: 'View this property listing.',
    });
  }
  ...
  ngOnInit(): void {
    this.route.params
      ...
      .subscribe((params) => {
        if (params['id']) {
          this.getPropertyListingDetails(params['id']);
        }
      });
  }

  getPropertyListingDetails(id: string): void {
      ...
      .subscribe({
        next: (res) => {
          const filtered = res.data.data
            .filter((prop) => prop._id !== currentListing._id)
            .slice(0, 3);
    ...
    if (this.isBrowser && navigator.share) {
      ...
      if (listing) {
        navigator.share({
          ...
          text: listing.property.description,
  ...
  getPropertyImages(): PropertyImage[] {
    const listing = this.listing();
    if (listing && listing.property && Array.isArray(listing.property.images)) {
      // Only include images with approvalStatus === 'APPROVED'
      const approvedImages: PropertyImage[] = listing.property.images.filter(
  ...
  getPropertyImagesWithCategory(): {
    ...
    const listing = this.listing();
    if (!listing || !listing.property || !listing.property.images) return [];

    return listing.property.images.map((img) => ({
      image: img,
      categoryName:
